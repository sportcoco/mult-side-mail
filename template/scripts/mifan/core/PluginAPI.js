...
const utils = require('@bomijs/utils');
const Logger = require('./Logger');
const { isValidPlugin, pathToObj } = require('./utils/pluginUtils');
const { ServiceStage, PluginType, EnableBy } = require('./enums');
...
module.exports = class PluginAPI {
  constructor(opts = {}) {
    ...
    this.utils = utils;
    this.logger = new Logger({ prefix: `>>  ${this.id || this.key}` });
  }
  ...
  describe({ id, key, config, enableBy }) {
    ...
    if (id && this.id !== id) {
      if (plugins[id]) {
        const name = plugins[id].isPreset ? PluginType.preset : PluginType.plugin;
